    ...
    while True:
        ...
        if __isRunning:
            if line_centerx != -1 and not obstacle:
                ...
                Board.setMotor(1, int(50 + base_speed))  # Left-front motor
                Board.setMotor(2, int(50 - base_speed))  # Right-front motor
                Board.setMotor(3, int(50 + base_speed))  # Left-rear motor
                Board.setMotor(4, int(50 - base_speed))  # Right-rear motor
...
def run(img, __target_color):
    ...
    if __isRunning:
        ...
        global obstacle
        if 0 < dist < 20:          # dist is in cm; 20 cm is a safe threshold
...
if __name__ == '__main__':
    ...
    __target_color = ('green',) # <--- Consider changing this to 'blue' for the lab track
    while __isRunning:
        ret, img = cap.read()
        if ret:
            frame = img.copy()
